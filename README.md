...
> 请整理 1.0.11 到 1.0.12 的版本更新，只写用户能感知的功能变化，不披露技术细节。默认生成 3 张小红书卡片图和公众号内容，实际调用图片服务出图，打包进 App，构建 APK，并通过 adb 覆盖安装后拉起手机上的 App。除非遇到缺少图片 API 配置或没有连接设备，否则不要停在文案、prompt 或 dry-run 阶段。

就可以自动走完：采集 → 审稿 → 规划卡片 → 出图 → 打包 → 构建 → 安装到手机 的全流程。

其中「只写用户功能变化」是对文案内容的要求，不等于「只生成文字」。如果只想要文案，需要明确说「只要文案，不出图、不构建 APK」。
